...
describe ONIX::ImprintRepresenter do
  ...
  describe "should correctly convert to a string" do
    Given(:imp) { ONIX::Imprint.new.extend(ONIX::ImprintRepresenter).from_xml(doc) }
    Then { imp.extend(ONIX::ImprintRepresenter).to_xml.to_s.start_with? "<Imprint>" }
  end
  ...
  context "should provide write access to first level attributes" do
    ...
    describe :imprint_name= do
      When { imp.imprint_name = "Paulist Press" }
      Then { imp.extend(ONIX::ImprintRepresenter).to_xml.to_s.include? "<ImprintName>Paulist Press</ImprintName>" }
    end
    describe :name_code_type= do
      When { imp.name_code_type = 1 }
      Then { imp.extend(ONIX::ImprintRepresenter).to_xml.to_s.include? "<NameCodeType>01</NameCodeType>" }
    end
  end
